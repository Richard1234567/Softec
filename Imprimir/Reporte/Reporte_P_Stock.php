...
$insumo= $_GET['id'];
$resultado = mysql_query("SELECT * FROM insumo JOIN insumo_marca ON RELA_INS_MARCA = ID_INS_MARCA JOIN insumo_modelo ON RELA_INS_MODELO = ID_INS_MODELO JOIN categoria ON RELA_CATEGORIA = ID_CATEGORIA JOIN proveedor ON RELA_PROVEEDOR = ID_PROVEEDOR WHERE ID_INSUMO ='".$insumo."'") or mysql_error(($conexion));
