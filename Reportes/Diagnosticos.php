...
function Header()
{
	...
	$this->Text(20,14,utf8_decode('Diagnóstico'),0,'C', 0);
...
function Footer()
{
	...
	$this->Cell(100,10,utf8_decode('Diagnóstico'),0,0,'L');
...
join modelo on RELA_MODELO = RELA_MODELO join persona on RELA_PERSONA = ID_PERSONA where ID_DIAGNOSTICO = '".$diagnostico."'";
	
	$diagnostico = mysql_query($strConsulta);
	
	$fila = mysql_fetch_array($diagnostico);
	...
	$pdf->Ln(5);
	$pdf->Cell(0,18,utf8_decode('Diagnóstico'),0,1,'C');
	$pdf->SetMargins(20,20,20);
	$pdf->Ln(0);

	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(40,8,utf8_decode('Dirección: Fotheringham N°: 2165'));
	$pdf->Cell(250,8,utf8_decode('Fecha: ').$fila['FECHA_DIAGNOSTICO'],0,0,'C');
	$pdf->Ln(5);
	$pdf->Cell(40,8,utf8_decode('Tel/Cel:  (037)-4429030'));
	$pdf->Ln(10);
	$pdf->Cell(40,8,utf8_decode('Cliente: ' .$fila['PERSONA_APELLIDO'] . ', ' .$fila['PERSONA_NOMBRE']));
	$pdf->Ln(10);
	$pdf->Cell(40,8,utf8_decode('Diagnóstico Técnico: '));
	$pdf->Ln(10);
	//convertimos el texto a utf8
	$texto = utf8_decode($fila['DIAGNOSTICO_DESC']);
	 
	$pdf->MultiCell(190,5,$texto);
	$pdf->Cell(40,8,utf8_decode('Precio Total: $' .$fila['PRECIO_TOTAL']));
	$pdf->Ln(20);
